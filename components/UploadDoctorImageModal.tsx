...
import { uploadDoctorImage } from '@/app/actions/admin'

export function UploadDoctorImageModal({
  doctorId,
  doctorName,
  isOpen,
  onClose
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    const res = await uploadDoctorImage(doctorId, formData)
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-xl border border-gray-100 overflow-hidden flex flex-col">
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div>
            ...
            <p className="text-xs text-gray-500 font-medium">{doctorName}</p>
          </div>
